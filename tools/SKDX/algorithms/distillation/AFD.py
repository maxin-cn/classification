...
class AFD(nn.Module):
	...
	def __init__(self, in_channels, att_f):
		...
		self.downscale = torch.nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
	...
	def forward(self, fm_s, fm_t, iftea, eps=1e-6):
		...
		rho = rho / torch.sum(rho, dim=0, keepdim=True)
		if iftea == 1:
			fm_t = self.downscale(fm_t)
		elif iftea == 2:
			fm_s = self.downscale(fm_s)

		fm_spow = torch.pow(fm_s, 2)
		fm_s_norm = torch.sum(fm_spow, dim=(2,3), keepdim=True)
		#fm_s_norm = torch.norm(fm_s, dim=(2,3), keepdim=True)
		fm_s      = torch.div(fm_s, fm_s_norm+eps)

		fm_tpow = torch.pow(fm_t, 2)
		fm_s_norm = torch.sum(fm_tpow, dim=(2,3), keepdim=True)
