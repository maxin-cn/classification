...
from baseslim.thr_party.apis import init_random_seed, set_random_seed, train_mmcls_model
from baseslim.thr_party.models import build_algorithm
from baseslim.thr_party.utils import setup_multi_processes

def parse_args():
    parser = argparse.ArgumentParser(description='Train a model')
    parser.add_argument('config', help='train config file path')
    parser.add_argument('-cpm', '--compress_model', 
                    default=None, type = str, help='the model file after pruning')
...
def main():
    ...
    # new build the model including the compressed one
    if (cfg.get('compress_op') is not None) and (cfg.compress_op == "kd"):
        algorithm = build_algorithm(cfg.algorithm)
        algorithm.init_weights()
    else:
        if args.compress_model:
            model = torch.load(args.compress_model)
        else:
    ...
    try:
        logger.info(f'start to report compression info')
        if torch.cuda.current_device() == 0:
            report_compression_info(cfg)
        else:
            pass  # convert model only once
    except:
        logger.info(f'skip report compression info')
