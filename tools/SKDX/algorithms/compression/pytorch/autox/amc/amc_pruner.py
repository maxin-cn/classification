...
from SKDX.compression.compressor import Pruner
from SKDX.algorithms.compression.pytorch.autox.amc.channel_pruning_env import ChannelPruningEnv
from SKDX.algorithms.compression.pytorch.autox.amc.pruner_lib.agent import DDPG
from SKDX.algorithms.compression.pytorch.autox.amc.pruner_lib.utils import get_output_folder
...
class AMCPruner(Pruner):
    ...
    def __init__(
        ...
        print('=> Saving logs to ', self.output_dir)
        ...
        print('** Actual replay buffer size: %d' % (rmsize))
        ...
        while episode < num_episode:  # counting based on episode
            ...
            if done:  # end of episode
                ...
                for i, preserve_rate in enumerate(env.strategy):
                    ...
                    print('preserve_rate/{}'.format(i), preserve_rate, episode)

                self.text_writer.write('best reward: {}\n'.format(env.best_reward))
                self.text_writer.write('best policy: {}\n'.format(env.best_strategy))

                print('reward/last', final_reward, episode)
                print('reward/best', env.best_reward, episode)
                print('info/accuracy', info['accuracy'], episode)
                print('info/compress_ratio', info['compress_ratio'], episode)
                print('info/best_policy', str(env.best_strategy), episode)
                print('best reward: {}\n'.format(env.best_reward))
                print('best policy: {}\n'.format(env.best_strategy))
