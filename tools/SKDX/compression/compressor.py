...
import logging
import torch
from SKDX.compression import default_layers
...
class Compressor:
    ...
    def reset(self, checkpoint=None):
        ...
        for layer, config in self._detect_modules_to_compress():
            #_logger.warning(layer.name, config)
...
class QuantizerModuleWrapper(torch.nn.Module):
    def __init__(self, module, module_name, module_type, config, quantizer):
        ...
        if 'weight' in config['quant_types']:
            if not (_check_weight(self.module) or _check_rnn_weight(self.module)):
                _logger.warning('Module %s does not have parameter "weight"', self.name)
            elif _check_weight(self.module):
                #_logger.warning('is cnn module')
            ...
            """
            elif _check_rnn_weight(self.module):
                #_logger.warning('is rnn module')
                self.module.register_parameter('old_weight_ih_l0', torch.nn.Parameter(self.module.weight_ih_l0))
                self.module.register_parameter('old_weight_hh_l0', torch.nn.Parameter(self.module.weight_hh_l0))
                delattr(self.module, 'weight_ih_l0')
                delattr(self.module, 'weight_hh_l0')
                self.module.register_buffer('weight_ih_l0', self.module.old_weight_ih_l0)
                self.module.register_buffer('weight_hh_l0', self.module.old_weight_hh_l0)
            """
    def forward(self, *inputs):
        ...
        if 'weight' in self.config['quant_types'] and _check_weight_type(self.module):
            #_logger.warning("=========cnn=========")
            #_logger.warning(type(self.module))
            self.quantizer.quant_grad.apply(
                self.module.old_weight,
                QuantType.QUANT_WEIGHT,
                self)
            #self.module.weight = self.module.weight.half()
            result = self.module(*inputs)
            """
            elif 'weight' in self.config['quant_types'] and _check_rnn_weight(self.module):
                #_logger.warning("=========rnn=========")
                #_logger.warning(type(self.module))
                self.quantizer.quant_grad.apply(
                    self.module.old_weight_ih_l0,
                    QuantType.QUANT_WEIGHT,
                    self)
                result = self.module(*inputs)
            """
        else:
            #_logger.warning("=========others=========")
            #_logger.warning(type(self.module))
            result = self.module(*inputs)
        ...
        if self.optimizer is not None:
            ...
            for wrapper in self.get_modules_wrapper():
                if 'weight' in wrapper.config['quant_types']:
                    ...
                    if hasattr(wrapper.module, 'old_weight'):
                        self.optimizer.add_param_group({"params": wrapper.module.old_weight})
                    """
                    if hasattr(wrapper.module, 'old_weight_ih_l0'):
                        self.optimizer.add_param_group({"params": wrapper.module.old_weight_ih_l0})
                    if hasattr(wrapper.module, 'old_weight_hh_l0'):
                        self.optimizer.add_param_group({"params": wrapper.module.old_weight_hh_l0})
                    """
...
def _check_rnn_weight(module):
    try:
        rnn_flag1 = isinstance(module.weight_ih_l0.data, torch.Tensor)
        rnn_flag2 = isinstance(module.weight_hh_l0.data, torch.Tensor)
        flags = (rnn_flag1 and rnn_flag2)
        return flags
    except AttributeError:
        return False

def _check_weight_type(module):
    try:
        cnn_flag1 = isinstance(module.weight.data, torch.Tensor)
        cnn_flag2 = type(module) == torch.nn.Conv2d
        flags = (cnn_flag1 and cnn_flag2)
        return flags
    except AttributeError:
        return False
"""
def _check_rnn_weight_type(module):
    try:
        rnn_flag1 = isinstance(module.weight_ih_l0.data, torch.Tensor)
        rnn_flag2 = isinstance(module.weight_hh_l0.data, torch.Tensor)
        flags = (rnn_flag1 and rnn_flag2)
