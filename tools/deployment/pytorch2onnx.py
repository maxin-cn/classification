...
def report_convertion_info(config):
    import json
    from tools import read_json, write_json, runcmd
    codebase_info = read_json("mtcvzoo")
    codebase_info["convertion"] = {"pytorch2onnx": True}
    codebase_info["training"] = {
                                    "network": config.model["backbone"]["type"],
                                    "task_type":"classification"
                                }
    print("codebase_info:", codebase_info)
    write_json("convertion_info.json", codebase_info)
    binary = "python tools/deployment/report_codebase_info.py"
    codebase_params = "--codebase_params_file convertion_info.json"
    command = " ".join((binary, codebase_params))
    if runcmd(command):
        print("report_convertion_info success")
    else:
        print("report_convertion_info fail")
...
if __name__ == '__main__':
    ...
    if args.checkpoint:
        ...
        if type(classifier.backbone) is TIMMBackbone and args.pretrained:
            checkpoint = _my_load_checkpoint(classifier, args.checkpoint, map_location='cpu', prefix='timm_model.', load_head=True)
        else:
            checkpoint = load_checkpoint(classifier, args.checkpoint, map_location='cpu')
