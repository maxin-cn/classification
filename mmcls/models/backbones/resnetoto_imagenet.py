...
class ResNetOTOImageNet(BaseBackbone):
    ...
    def __init__(self, 
        ...
        super(ResNetOTOImageNet, self).__init__(init_cfg)
        ...
        if self.cfg is None:
            ...
            self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
            self.fc = nn.Linear(cfg[4][-1][-1], self.num_classes)
        ...
        def init_weights(self):
            super(ResNetOTOImageNet, self).init_weights()
    ...
    def get_parameters(self):
        return self.optimizer_grouped_parameters
    ...
    def prune(self):
        ...
        prune_model = self.resnet50(cfg=prune_cfg, num_classes=self.num_classes)
    ...
    def _resnet(self, arch, block, layers, pretrained, last_model_path='', num_classes=1000, **kwargs):
        model = ResNetOTOImageNet(block, layers, num_classes, **kwargs)
        if pretrained:
    ...
    def resnet50(self, pretrained=False, last_model_path='', num_classes=1000, **kwargs):
        return self._resnet('resnet50', Bottleneck, [3, 4, 6, 3], pretrained, last_model_path, num_classes,
                    **kwargs)
