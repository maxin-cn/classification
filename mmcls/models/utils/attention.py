    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            if torch.onnx.is_in_onnx_export():
                '''
                print("attn0:", attn.shape)
                print("mask:", mask.shape)
                print("mask.unsqueeze(1).unsqueeze(0):", mask.unsqueeze(1).unsqueeze(0).shape)
                print("B_ // nW:", B_ // nW)
                print("nW:", nW)
                ...
                '''
                # B_//nW的值为1，可以将view算子输入修改为4维，同时mask不再做unsqueeze(0)操作
                attn = attn.view(-1, self.num_heads, N, N) + mask.unsqueeze(1)
            else:
                attn = attn.view(B_ // nW, nW, self.num_heads, N,
                             N) + mask.unsqueeze(1).unsqueeze(0)
